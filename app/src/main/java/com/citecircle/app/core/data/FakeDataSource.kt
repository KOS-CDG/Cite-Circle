...
object FakeDataSource {
    ...
    val sampleWorkspace = CircleWorkspace(
        id = "ws_default",
        circleId = "",
        pinBoardText = "📌 **Workspace Pin Board**\n\nPin announcements or guidelines here.",
        accessType = CircleAccessType.PUBLIC,
        inviteCode = ""
    )

    val sampleMembers: List<CircleMember> = emptyList()

    val sampleDrafts: MutableList<CircleDraft> = mutableListOf()

    val sampleReviewRequests: MutableList<DraftReviewRequest> = mutableListOf()

    val sampleDraftComments: MutableList<DraftComment> = mutableListOf()

    val sampleReadingLists: MutableList<CircleReadingList> = mutableListOf()
}
